...
// CalcWaysWinsInReels -
func CalcWaysWinsInReels(paytables *sgc7game.PayTables, rss *ReelsStats, symbol SymbolType, num int, height int) (int64, error) {
	curwins := int64(1)

	for i := 0; i < num; i++ {
		curwins *= int64(rss.GetScatterNum(i, symbol, IRSTypeSymbol, height))
	}

	for i := num; i < len(rss.Reels); i++ {
		curwins *= int64(rss.GetScatterNum(i, symbol, IRSTypeNoSymbol, height))
	}
	...
	for _, s := range symbols {
		...
		if isok {
			for i := 0; i < len(arrPay); i++ {
				if arrPay[i] > 0 {
					cw, err := CalcWaysWinsInReels(paytables, rss, s, i+1, height)
					if err != nil {
