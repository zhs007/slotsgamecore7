...
func (symbolMulti *SymbolMulti) InitEx(cfg any, pool *GamePropertyPool) error {
	...
	if len(symbolMulti.Config.MapWeightMulti) > 0 {
		...
		for k, v := range symbolMulti.Config.MapWeightMulti {
			vw2, err := pool.LoadValWeights(v, "val", "weight", sgc7game.NewIntVal[int], symbolMulti.Config.UseFileMapping)
			if err != nil {
		...
		vw2, err := pool.LoadValWeights(symbolMulti.Config.WeightMulti, "val", "weight", sgc7game.NewIntVal[int], symbolMulti.Config.UseFileMapping)
		if err != nil {
			goutils.Error("SymbolMulti.Init:LoadValWeights",
