...
func (symbolVal2 *SymbolVal2) Init(fn string, pool *GamePropertyPool) error {
	...
	if cfg.WeightSet != "" {
		vw2, err := sgc7game.LoadValWeights2FromExcel(cfg.WeightSet, "val", "weight", sgc7game.NewIntVal[int])
		if err != nil {
	...
	for _, v := range cfg.WeightsVal {
		vw2, err := sgc7game.LoadValWeights2FromExcel(v, "val", "weight", sgc7game.NewIntVal[int])
		if err != nil {
			goutils.Error("SymbolVal2.Init:LoadValWeights2FromExcel",
				zap.String("Weight", v),
				zap.Error(err))
